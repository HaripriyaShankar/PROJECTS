<!DOCTYPE html>
<html>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
<link rel="stylesheet" href="style4.css">
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
</head>
<body>
<div class="header_img">
 <a href=""><img class="img_head" src="data:image/png;base64,"></a>
  <br><div class="para" style="color:#5b5e56">RAILWAY&nbspRESERVATION&nbspSYSTEM</div>
  <a href=""><img class="imge" src="data:image/jpeg;base64,"></a>
</div>

<div class="topnav" id="myTopnav">
  <a href="home.php">HOME</a>
  <a href="schedule.php">TRAIN SCHEDULE</a>
  <a href="booking.php">BOOK TICKET</a>
  <a href="cancel.php"  class="active3">CANCEL TICKET</a>
  <a href="login.php">LOGOUT</a>
  <a href="javascript:void(0);" class="icon" onclick="myFunction()">
    <i class="fa fa-bars"></i>
  </a>
</div>

<div style="padding-left:16px">
</div>

<div class="body_img"></div>
<div class="container2">
<form method="POST">
    <br><br>
    &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<label for="name">TICKET&nbspNUMBER</label>&nbsp&nbsp
        <input type="text" size="20"style="font-size:20px" name="tickno" required /><br> <br><br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
        &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<label for="name">TRAIN&nbspNUMBER</label>&nbsp&nbsp&nbsp
        <input type="text" size="20"style="font-size:20px" name="trainno" required /><br> <br><br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
<br>
&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
        <button type="submit" class="submitbook" name="submitbutton" style="font-size:20px">
    GO
        </button>
</form>
</div>

<?php
if(isset($_POST["submitbutton"])){  
$ticket=$_POST['tickno'];
$trano=$_POST['trainno'];
$con = mysqli_connect('localhost', 'root', '','railway');
$query=mysqli_query($con,"SELECT * FROM booking WHERE dbtickno='".$ticket."'");  
    $numrows=mysqli_num_rows($query);  
    if($numrows!=0)  {
    $sql="DELETE from booking where dbtickno=$ticket";
    $rs = mysqli_query($con, $sql);
    echo '<script type="text/javascript">alert("YOUR TICKET HAS BEEN SUCCESSFULLY CANCELLED");</script>';
    }
    else{
        echo '<script type="text/javascript">alert("ENTER YOUR TICKET NUMBER CORRECTLY");</script>'; 
    }

    $query1 = mysqli_query($con,"SELECT dbclass FROM booking WHERE dbtickno='$ticket'");
    $row1 = mysqli_fetch_row($query1);
    if($row1[9]=='AC FIRST CLASS'){
      echo 'print';
     $var="UPDATE schedule SET dbacfirstclass=dbacfirstclass+1 WHERE dbtrano=$trano";
      $res=mysqli_query($con,$var);
    }

    if($row1[9]=='FIRST CLASS'){
      $var="UPDATE schedule SET dbfirstclass=dbfirstclass+1 WHERE dbtrano=$trano";
      $res=mysqli_query($con,$var);
    }
    if($row1[9]=='SLEEPER'){
      $var="UPDATE schedule SET dbsleeperclass=dbsleeperclass+1 WHERE dbtrano=$trano ";
      $res=mysqli_query($con,$var);
    }
    }
?>

    <script>
function myFunction() {
  var x = document.getElementById("myTopnav");
  if (x.className === "topnav") {
    x.className += " responsive";
  } else {
    x.className = "topnav";
  }
}
</script>

</body>
</html>